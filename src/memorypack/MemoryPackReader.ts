...
export class MemoryPackReader {
    ...
    public readInt64(): Long {
        ...
        const left = this.dataView.getInt32(this.offset, true);
        const right = this.dataView.getInt32(this.offset + 4, true);

        // 将两个 32 位的值组合在一起
        // const combined = true ? left + 2 ** 32 * right : 2 ** 32 * left + right;
        const combined = new Long(left, right, false)
        this.offset += 8;
    ...
    public readNullableInt64(): Long | null {
        if (this.readInt64().isZero()) {
            this.offset += 8;
            return null;
        }
    ...
    public readUint64(): Long {
        // 将 64 位的数字拆分位两个 32 位（4 字节）的部分
        const left = this.dataView.getUint32(this.offset, true);
        const right = this.dataView.getUint32(this.offset + 4, true);

        // 将两个 32 位的值组合在一起
        // const combined = true ? left + 2 ** 32 * right : 2 ** 32 * left + right;
        const combined = new Long(left, right, true)
        this.offset += 8;
        if (!Number.isSafeInteger(combined))
            console.warn(combined, "超过 MAX_SAFE_INTEGER。可能存在精度丢失。");
        return combined;
    ...
    public readNullableUint64(): Long | null {
        if (this.readUint64().isZero()) {
            this.offset += 8;
            return null;
        }
    ...
    public readDate(): Date {
        ...
        const ticks = this.readUint64().and(dateTimeMask);
        // const unixMillisecond = (ticks - unixEpochTicks) / 10000n;
        const unixMillisecond = ticks.sub(unixEpochTicks).div(10000);
